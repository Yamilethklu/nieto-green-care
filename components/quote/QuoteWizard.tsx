...
export function QuoteWizard({ locale, services }: { locale: Locale; services: Service[] }) {
  ...
  const [form, setForm] = useState({
    ...
    service_id: services.find(service => service.slug === "lawn-mowing")?.id || services[0]?.id || "", frequency: "biweekly" as Frequency, occupancy: "occupied" as "occupied" | "vacant",
    mow_area: "front_back" as MowArea, corner_lot: "no" as YesNo, date: "", time: "09:00",
    optional_services: [] as string[], first_name: "", last_name: "", email: "", phone: "", is_cellphone: "yes" as YesNo,
  ...
  useEffect(() => {
    if (step !== 4 || !form.service_id || form.sqft <= 0) return;
    const controller = new AbortController();
    fetch(`/api/availability?service=${encodeURIComponent(form.service_id)}&frequency=${form.frequency}&sqft=${Math.round(form.sqft)}`, { signal: controller.signal })
      .then(response => response.ok ? response.json() : Promise.reject())
      .then(data => { setFullDates(data.fullDates || []); setEstimatedPrice(data.estimatedPrice ?? null); })
      .catch(() => { if (!controller.signal.aborted) { setFullDates([]); setEstimatedPrice(null); } });
    return () => controller.abort();
  }, [step, form.service_id, form.frequency, form.sqft]);
  const calendarDays = useMemo(() => Array.from({ length: 28 }, (_, offset) => {
    const date = new Date();
    date.setHours(12, 0, 0, 0);
    date.setDate(date.getDate() + offset + 1);
    const iso = date.toISOString().slice(0, 10);
    const weekend = date.getDay() === 0 || date.getDay() === 6;
    const full = fullDates.includes(iso);
    return { iso, weekend, full, day: date.toLocaleDateString(locale === "es" ? "es-US" : "en-US", { weekday: "short", day: "numeric" }) };
  }), [locale, fullDates]);
  ...
  const summary = useMemo<BookingSummary>(() => ({
    ...
    address: form.address, sqft: form.sqft, estimate: estimatedPrice, date: form.date, time: form.time,
    frequency: form.frequency, payment: form.payment,
  }), [es, form, selected, estimatedPrice]);
  ...
  async function submit() {
    ...
    const payload = {
      ...
      property_occupancy: form.occupancy, mow_area: form.mow_area, corner_lot: form.corner_lot === "yes", is_cellphone: form.is_cellphone === "yes",
  ...
  if (done && step === 8) return (
    <div className="grid">
      ...
      <PlanSummary booking={done} locale={locale} optionalServices={form.optional_services} mowArea={form.mow_area} latitude={form.lat} longitude={form.lng} />
  ...
  return (
    <div className="quote-shell">
      ...
      <div className="card quote-card">
        ...
        {step === 4 && <section><div className="eyebrow">{es ? "Paso 4 · Calendario" : "Step 4 · Calendar"}</div><h2 className="section-title">{es ? "Elija una fecha de inicio" : "Choose a preferred start date"}</h2><p className="muted">{es ? "La fecha y el precio por visita se confirmarán después de revisar la propiedad y la capacidad de la ruta." : "The date and per-visit price are confirmed after property and route-capacity review."}</p><div className="calendar-panel"><div className="quote-calendar" role="group" aria-label={es ? "Fechas de inicio disponibles para solicitar" : "Start dates available to request"}>{calendarDays.map(day => <button type="button" key={day.iso} disabled={day.weekend || day.full} aria-pressed={form.date === day.iso} className={`calendar-day ${day.full ? "full" : ""} ${form.date === day.iso ? "selected" : ""}`} onClick={() => setForm({ ...form, date: day.iso })}><strong>{day.day}</strong><small>{day.full ? (es ? "Completo" : "Full") : day.weekend ? (es ? "Cerrado" : "Closed") : estimatedPrice !== null ? `$${estimatedPrice.toFixed(2)}` : (es ? "Cotizar" : "Quote")}</small></button>)}</div><div className="field"><label htmlFor="time">{es ? "Hora preferida" : "Preferred time"}</label><input id="time" type="time" className="input" value={form.time} onChange={event => setForm({ ...form, time: event.target.value })} /></div><div className="capacity-key"><span><i className="available" />{es ? "Disponible para solicitar" : "Available to request"}</span><span><i className="pending" />{es ? "Las fechas rojas están a capacidad completa" : "Red dates are currently at full capacity"}</span></div></div></section>}
        ...
        {step === 6 && <section><div className="eyebrow">{es ? "Paso 6 · Cuenta" : "Step 6 · Account details"}</div><h2 className="section-title">{es ? "Cuéntenos los detalles" : "Tell us the details"}</h2><div className="form-grid">{[["first_name",es ? "Nombre" : "First name","text"],["last_name",es ? "Apellido" : "Last name","text"],["email","Email","email"],["phone",es ? "Teléfono" : "Phone","tel"]].map(([key,label,type]) => <div className="field" key={key}><label htmlFor={key}>{label}</label><input id={key} required type={type} className="input" value={form[key as "first_name"]} onChange={event => setForm({ ...form, [key]: event.target.value })} /></div>)}</div><fieldset className="yes-no-row"><legend>{es ? "¿Es un teléfono celular?" : "Is it a cellphone?"}</legend>{(["yes","no"] as const).map(value => <label key={value}><input type="radio" name="cellphone" checked={form.is_cellphone === value} onChange={() => setForm({ ...form, is_cellphone: value })} /> {value === "yes" ? (es ? "Sí" : "Yes") : "No"}</label>)}</fieldset><div className="terrain-questions">{questions.map(([key,en,spa]) => <fieldset className="yes-no-row" key={key}><legend>{es ? spa : en}</legend>{(["yes","no"] as const).map(value => <label key={value}><input type="radio" name={key} checked={form.answers[key] === value} onChange={() => setForm({ ...form, answers: { ...form.answers, [key]: value } })} /> {value === "yes" ? (es ? "Sí" : "Yes") : "No"}</label>)}</fieldset>)}</div><div className="field"><label>{es ? "Solicitudes especiales" : "Special requests"}</label><textarea className="input" maxLength={1500} value={form.special_requests} onChange={event => setForm({ ...form, special_requests: event.target.value })} /></div><div className="field"><label>{es ? "¿Cómo nos conoció?" : "How did you hear about us?"}</label><select className="input" value={form.referral_source} onChange={event => setForm({ ...form, referral_source: event.target.value })}><option value="">{es ? "Seleccione…" : "Select…"}</option><option value="search">{es ? "Búsqueda web" : "Web Search"}</option><option value="social">{es ? "Redes sociales" : "Social"}</option><option value="referral">{es ? "Recomendación" : "Referral"}</option><option value="mail_card">{es ? "Tarjeta por correo" : "Mail Card"}</option><option value="yard_sign">{es ? "Letrero" : "Yard sign"}</option><option value="other">{es ? "Otro" : "Other"}</option></select></div><input name="website" aria-hidden="true" tabIndex={-1} autoComplete="off" className="honeypot" value={form.website} onChange={event => setForm({ ...form, website: event.target.value })} /></section>}
        {step === 7 && <section><div className="eyebrow">{es ? "Paso 7 · Confirmación" : "Step 7 · Confirmation"}</div><h2 className="section-title">{es ? "Revise y envíe su solicitud" : "Review and submit your request"}</h2><PlanSummary booking={summary} locale={locale} optionalServices={form.optional_services} mowArea={form.mow_area} latitude={form.lat} longitude={form.lng} /><div className="field payment-preference"><label>{es ? "Preferencia de pago" : "Payment preference"}</label><select className="input" value={form.payment} onChange={event => setForm({ ...form, payment: event.target.value as "cash" | "transfer" })}><option value="cash">{es ? "Efectivo al recibir el servicio" : "Cash on Service"}</option><option value="transfer">{es ? "Transferencia (Cash App o Zelle)" : "Transfer (Cash App or Zelle)"}</option></select><p className="payment-warning">{es ? "NO PAGUE hasta que Nieto Green Care confirme el servicio y el precio final." : "DO NOT PAY until service and final price are confirmed by Nieto Green Care."}</p></div><Turnstile locale={locale} onToken={token => setForm(current => ({ ...current, captcha_token: token }))} /></section>}

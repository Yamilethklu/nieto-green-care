...
- **Gallery:** open `/admin/gallery`, upload authorized real work as JPEG, PNG, or WebP up to 5 MB, add meaningful alt text and bilingual captions, then publish. The server validates the admin session and removes the storage object if metadata cannot be saved.
...
The dashboard provides secure database-backed operational views and a reviewed gallery upload form. Other complex mutations should be made through Supabase Studio under an admin account until dedicated forms are enabled; RLS remains enforced. Confirm destructive actions and retain backups.

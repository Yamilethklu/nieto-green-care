...
2. Add every `.env.example` variable to Preview/Production. Use the production URL (currently provisional `https://proyecjardin.vercel.app`, later the confirmed domain) for `NEXT_PUBLIC_APP_URL` with no trailing slash.
   Confirm `NEXT_PUBLIC_CASH_APP_URL` and `NEXT_PUBLIC_ZELLE_PHONE` with the business owner before enabling customer payments.
...
Create a Cloudflare Turnstile widget for the final production hostname. Set `NEXT_PUBLIC_TURNSTILE_SITE_KEY` and the server-only `TURNSTILE_SECRET_KEY`. When the secret is configured, the lead endpoint rejects submissions without a successful server-side verification. Preview/local environments may omit both variables.

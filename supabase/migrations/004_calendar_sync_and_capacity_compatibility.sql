...
-- Safe to run against the existing production schema even when migration 002 was not applied.
...
-- Preserve appointments that predate explicit capacity configuration.
WITH counts AS (
...
SET max_slots=GREATEST(sc.max_slots, counts.scheduled_count), booked_slots=counts.scheduled_count
FROM counts
...
-- Keep the legacy is_full flag compatible until old production code is retired.
CREATE OR REPLACE FUNCTION public.sync_schedule_legacy_flags() RETURNS trigger
LANGUAGE plpgsql SET search_path='' AS $$
BEGIN
  IF NEW.is_blocked IS DISTINCT FROM OLD.is_blocked THEN NEW.is_full := NEW.is_blocked;
  ELSIF NEW.is_full IS DISTINCT FROM OLD.is_full THEN NEW.is_blocked := NEW.is_full;
  ELSE NEW.is_full := NEW.is_blocked;
  END IF;
  RETURN NEW;
END;$$;
...
-- Recalculate capacity after INSERT, UPDATE, or DELETE.
CREATE OR REPLACE FUNCTION public.sync_schedule_capacity() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
BEGIN
  IF TG_OP='DELETE' THEN
    IF OLD.requested_date IS NOT NULL THEN PERFORM public.recalculate_booked_slots(OLD.requested_date); END IF;
    RETURN OLD;
  END IF;
...
AFTER INSERT OR DELETE OR UPDATE OF status,requested_date ON public.leads
